...
def CIK_update():
    with open('edgar_mapping/CompanyCIK.json', 'w') as cj:
        r = loads(get(
...
def CIK_map(t, f=False):
    t = t.upper()
    if f:
        with open('edgar_mapping/MFCIK.json', 'r') as mf:
            try:
                cik = loads(mf.read())[t]
                return cik
            except KeyError:
                return {}
    with open('edgar_mapping/CompanyCIK.json', 'r') as cj:
        try:
            cik = loads(cj.read())[t]
